...
return static function (DefinitionConfigurator $definition): void {
    $definition->rootNode()
        ->children()
            ->scalarNode('theme')
                ->info('Component style theme (a directory of config/styles/). Themes inherit the "default" theme for anything they do not redefine.')
                ->defaultValue('default')
                ->cannotBeEmpty()
            ->end()
            ->arrayNode('components')
                ->info('CVA styles per component, merged over the bundle defaults (config/styles/*.php).')
